...
class Agent:
    ...
    def check_self_status(self):
        # if own model is not in PB and not at block limit
        if (len(self.pb) < self.L) and (self.pb.get(self.my_ID) is None):
        ...
        if len(self.pb) == self.L:  # close block
            ...
            # only update stats if i'm first
            if completed_block_count < len(self.chain):
    ...
    def broadcast(self):
        for nnode in G[self.my_ID]:
            G.nodes[nnode]['agent'].receive_chain(self.chain, self.pb)
        self.cur_epoch += 1
    ...
    def send_direct_chain(self, reference_pb):
        if self.pb == reference_pb:
            return
        # sleep a bit - don't do this every epoch
        self.last_absorb_epoch = self.cur_epoch
        missing_ids = np.setdiff1d(np.array(list(reference_pb.keys()), dtype=int), np.array(
            list(self.pb.keys()), dtype=int), assume_unique=True)
        if len(missing_ids) > 0:
            sample = np.random.choice(missing_ids, np.min(
                (3, len(missing_ids))), replace=False)
            # send 'broadcast' to missing nodes (direct messages) suggesting my pb
            for id in sample:
...
class DES:
    ...
    def run(self, num_blocks=1, print_progress=True):
        ...
        print("Running DES to simulate", self.n,
              "agents over", num_blocks, "blocks:")
        print("L:", self.L)
        ...
        while completed_block_count < num_blocks:
            ...
            # next broadcast is in broadcast_interval + noise
            self.Q.put((cur_time + self.broadcast_interval +
                        np.random.rand()*.1, agent_id))
            ...
            if last_block_count < completed_block_count:
                ...
                epochs_total += int((cur_time-last_block_time) //
                                    self.broadcast_interval)
                total_exp += (cur_time-last_block_time)*self.L
                if print_progress:
                    print("(t:"+display_time(cur_time)+") Block #" + str(completed_block_count) +
                          " has been closed by agent " + str(completed_block_agent))
                    print("      It took", display_time(cur_time-last_block_time), "and", int(np.ceil(
                        (cur_time-last_block_time)/self.broadcast_interval)), "epochs to close the block.")
                    print("      The current Global Model has",
                          display_time(total_exp), "experience.")
                    print("      There were", dir_message_count,
                          "direct messages sent over the network")
        ...
        print("Avrg. time per block: ", display_time(
            cur_time/completed_block_count))
        print("Avrg. number of epochs per block: ", np.round(
            epochs_total/completed_block_count, 2))
        print("Years experience in current Global Model: ",
              display_time(total_exp))
        print("Avrg. number of direct messages sent per block: ",
              np.round(dir_message_total/completed_block_count, 2))
